...
namespace
{
	 ...
	 * "these twelve assets are broken by one renamed function".
	 */
	FString CauseOf(const FString& ErrorText)
	{
		...
		{
			return FString::Printf(TEXT("missing function or changed signature: %s"),
	...
	/** Blueprint assets under a path, oldest API first (deterministic order). */
	...
	Registry.RegisterQuick(
		...
		[](const FUplinkToolContext& Ctx) -> FUplinkToolResult
		{
			...
			for (const FAssetData& Asset : Assets)
			{
				...
				{
					const FString Cause = CauseOf(ErrorText);
			...
			Data->SetBoolField(TEXT("truncated"), Assets.Num() > Checked);
			if (bIncludeWarnings)
			{
				Data->SetNumberField(TEXT("withWarnings"), WithWarnings);
			}

			return FUplinkToolResult::Ok(Data, Broken == 0
				? FString::Printf(TEXT("all %d Blueprints compiled"), Checked)
				: FString::Printf(TEXT("%d of %d Blueprints broken, from %d distinct cause(s) - bp_repair fixes the mechanical ones"),
	...
	Registry.RegisterQuick(
		...
		[](const FUplinkToolContext& Ctx) -> FUplinkToolResult
		{
			...
			if (Targets.Num() == 0)
			{
				...
				return FUplinkToolResult::Ok(None, TEXT("nothing broken to repair"));
			}
			...
			if (bDryRun)
			{
				...
				return FUplinkToolResult::Ok(Plan, FString::Printf(
