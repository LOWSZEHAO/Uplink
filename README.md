...
**v0.22.0 — VR play-testing without a headset, and calls that cannot lie.** 95 tools across every layer, one codebase compiling against both **UE 5.7 and 5.8**, every capability verified against a live editor and running game. `xr_simulate` drives a VR pawn+s head and hands with **no headset attached**, so VR interactions can be play-tested from a desk - it leans on the engine keeping an untracked controller+s pose instead of resetting it, and `reach` puts a hand on a target actor so a grab needs no coordinate maths. It also tells VR and desktop apart, so a pawn that fell back to desktop cannot silently pass a VR test. Calls themselves are now checked: a misspelt parameter is refused with a suggestion instead of being ignored, and a backgrounded editor is redrawn before capture rather than handing back a stale frame. Edits made through Uplink are undoable, durable and honest: every mutating tool runs inside its own named editor transaction so an agent's change can be rolled back like a hand edit (`edit_history`), authored work can be written to disk (`save`) instead of quietly living in memory until the editor closes, an asset path that resolves to nothing is refused rather than written as null, and tools that used to report success without doing anything now say what went wrong and what the valid options were. Terrain can now come from real elevation data: public-domain DEM tiles decode into `landscape_create` heightmaps, CC0 PBR texture sets import via `asset_import`, and a slope-blended landscape material gets authored node-by-node over `call_function` — the Grand Canyon demo map is built from the actual USGS-derived elevation of the canyon. `spawn_batch` places up to 1000 actors in one call for scene assembly at scale. Graphs it authors follow strict style rules: nodes never overlap, exec chains lay out as straight horizontal lanes, and turning wires get reroute knots so runs stay level and clean (`bp_modify` `arrange`; wire rendering is stock Unreal). Remaining before 1.0: broader real-project mileage and API polish from feedback.

**The toolset (95 tools):**
...
- **Virtual reality:** `xr_simulate` — pose the HMD and either hand, or `reach` for an actor, with no headset connected. Untracked motion controllers keep whatever pose they are given, so simulated hands stay put and any overlap-based interaction (grabs, triggers, hand-proximity UI) can be driven and asserted on from a script. Buttons come from `input_action`.
...
- **Ask the world:** `trace` (line/sphere/box/capsule casts by channel or collision profile — ground height, line of sight, whether a capsule fits, what a hand would overlap; hits come back with actor, component, impact point, normal, distance and physical material) · `ai_query` (every AI controller's pawn, behaviour tree, **currently active node**, task chain and full blackboard — the active node is not exposed to Blueprint at all) · `material_query` (a material's expressions, connected inputs, parameters and, above all, its **compile errors** — the answer to why a surface renders black)
...
- **Blueprints:** `bp_create` · `bp_query` (variables, components, graphs, nodes, pins, connections) · `bp_add_component` (build a component hierarchy — meshes, triggers, scene parents — with transforms, mesh assignment and collision profiles) · `bp_modify` (variables; **function graphs** via `add_function`, including thread-safe ones needed for anim-graph node functions; call-function / custom-event / engine-event / component-bound-event / variable get-set nodes; wiring; pin defaults; `remove_function`; `set_node_property` to set any property on a graph node by guid, dotted paths included — **batchable: a whole event graph in one call** via `ops` with `@ref` node handles, and a failed batch reports how far it got) · `bp_compile` (full diagnostics)
...
- **Editor & world:** `status` · `console_command` · `output_log` (incremental log reads — pair with `pie_status.log_start_index` to read just the current session) · `viewport_screenshot` (PNG image results) · `viewport_camera` (frame an actor or set the editor camera before capturing) · `level_actors` · `spawn_actor` · `spawn_batch` (up to 1000 actors in one call — meshes with material overrides or any actor class) · `delete_actors` · `move_actor` · `live_compile` (Live Coding patch of the running editor — C++ iteration without restarts) · `edit_history` (inspect and walk the undo stack — every mutating tool is its own named transaction, so agent edits undo exactly like hand edits)
- **Reflection:** `get_property` / `set_property` (any UPROPERTY as JSON; both take dotted paths like `RootComponent.RelativeLocation.X`, stepping through structs *and* object references, so a path can cross from an actor into a component and on into its structs) · **`call_function`** (any UFUNCTION — args in, return value and out-params back as JSON). A named object reference that resolves to nothing is refused rather than silently written as null.
- **Assets:** `asset_search` / `asset_dependencies` / `asset_referencers` / `asset_import` (bring FBX/textures/audio from disk into the project, automated) · `save` (write edited assets to disk — authoring leaves work in memory, and an editor restart discards anything unsaved)
