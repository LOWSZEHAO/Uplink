...
**v0.38.2 — 120 tools, one codebase compiling against UE 5.7 and 5.8 (Win64, editor builds).** The scenario suite in [`scenarios/`](scenarios) runs clean on the third-person template for both engines, and one command runs it against your own project.
...
- **There is no reflection escape hatch.** No arbitrary UFUNCTION call, and properties resolve by top-level name only — no dotted paths through structs and object references, so a Slate brush's tint or a post-process setting is out of reach. It does have `execute_tool_script`, which batches several of its own tool calls into one round trip, but that is a batching mechanism rather than a way out: the script sandbox allows `json`, `math`, `datetime`, `copy`, `re` and `time`, and cannot import `unreal`. `call_function`, `get_property` and `set_property` are how you reach a system nobody has written a tool for yet.
...
Where Epic is ahead, it is ahead, and it is worth being specific about. Blueprint graph authoring is a real contest, and their DSL is a genuinely different approach to it — you hand it one script rather than a batch of node and wire ops. Its UMG toolset does tree surgery this one does not — wrap, replace-with-template, rename, UI components — though it has no way to set a slot's position or size, because its property access resolves top-level names only. It ships a Gameplay Ability System toolset, reading a pawn's granted abilities, active effects and attribute values; there is nothing here for that. Niagara, PCG and Slate inspection are all deeper there, and the Slate tools do things this plugin deliberately stopped trying to. Its Behaviour Tree and StateTree toolsets are read-only in 5.8 — `list_nodes`, `get_children`, `get_blackboard` — so neither server authors one of those graphs.
