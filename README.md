...
- **Blueprints:** `bp_create` · `bp_query` (variables, components, graphs, nodes, pins, connections) · `bp_add_component` (build a component hierarchy — meshes, triggers, scene parents — with transforms, mesh assignment and collision profiles) · `bp_modify` (variables; **function graphs** via `add_function`, including thread-safe ones needed for anim-graph node functions; call-function / custom-event / engine-event / component-bound-event / variable get-set nodes; wiring; pin defaults; `set_node_property` to set any property on a graph node by guid, dotted paths included — **batchable: a whole event graph in one call** via `ops` with `@ref` node handles) · `bp_compile` (full diagnostics)
...
- **Reflection:** `get_property` (any UPROPERTY as JSON, with dotted paths like `MyStruct.Inner.Value` to reach struct members) / `set_property` · **`call_function`** (any UFUNCTION — args in, return value and out-params back as JSON)
