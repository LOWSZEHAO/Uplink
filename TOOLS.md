...
| `bp_query` | Parent class, compile status, variables, and per-graph nodes with pins/defaults/connections. Node guids are the handles `bp_modify` uses. Big graphs page: `offset` skips into a graph, `total_nodes` reports its true size, `next_offset` is the cursor for the next call. `{blueprint, graph?, max_nodes?, offset?}` |
