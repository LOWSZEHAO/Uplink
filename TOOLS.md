...
| `set_pin_default` | Literal pin values, object-aware. An enum pin takes the authored entry name as well as the stored one — a User Defined Enum pin holds `NewEnumerator<n>`, which is not a name anyone has, so a value is translated before the schema sees it. Verified after writing: a value the pin's type will not take is refused rather than silently dropped. |
