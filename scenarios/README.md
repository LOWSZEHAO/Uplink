...
| `26-widget-tree-editing` | A widget can be moved out of the panel it was added to and removed when it is not wanted, and the layout is writable at all — `widget_tree` reports the object path of each widget and of the slot that positions it, and `set_property` writes both. Pins the three refusals: a misspelt name answered with the nearest real one, a reparent that would make the tree a ring, and a remove while a graph node still reads the widget. The last one matters more than it looks: the engine deletes those nodes along with the widget, so forcing it compiles clean and loses the logic silently. |
| `27-writes-that-do-nothing` | Two writes that land, read back, and change nothing — a struct handed keys that name none of its fields, and a value whose `bOverride_` flag is off. Neither is catchable after the fact: `set_property` verifies by re-applying the request to what is there, and a conversion that is a deterministic no-op reproduces itself exactly and agrees. Both are refused before the write, and the file pins the other half too — partial writes, the spelling that does work, and `force:true`. |
| `28-named-slots-and-bindings` | A named slot is declared in one blueprint and filled in another, and the engine's own listing walks the filled-slot map so it can never report the empty one you want to fill. This builds both halves - a base that declares, a child that inherits and fills - and then the property-binding preconditions: a property with no sibling `<Name>Delegate`, a function that does not exist, and one that is not pure. All three otherwise fail at compile time against the widget rather than the cause. |
